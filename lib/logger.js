...
module.exports={install:function(X){
	...
	function logger(opts) {
		...
		let $={tbl:X.U, o:o, dologging: 1, logStats:[], lastFlush: 0, timer:X.U, v:[]};
		let ext='.log.json';
		function emit(x,y) {
		...
		function flush() {
			...
			const t=X.now();
			const lfn=fn(t),nrecs=X.len($.v);
			emit([lfn,nrecs],'logger.flush');
			X.save(lfn, $.v);
			$.o.lastFlush=t; $.v=[];
		}
		function logcheck() {
		...
		function appender(type) {
			return function(x,y) { if ($.dologging) $.v.push([$.tbl.name,type,x,y]); }
		}
		$.ins=appender('ins');
		$.upd=appender('upd');
