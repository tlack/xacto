...
module.exports={install: function X(X){
	...
	*/
	...
	impose('choice dict drop equal first flip head get inter key last len');
	...
	function whereVal(col, x) {
		...
		if(typeof(x)==='object') 
			for(ci=0; ci<cl; ci++) if(X.equal(col[ci],x)===true) r.push(ci);
		else
			for(ci=0; ci<cl; ci++) if(col[ci]==x) r.push(ci);
		return r;
	}
