...
module.exports = {install: function filehandlers(X){
	...
	function dfl_load(fn,cb){ return read(fn,cb); }
	function dfl_save(fn,x,cb){ return write(fn,x,cb); }
	function dfl_imp(str, cb, opts) {
		if (cb) return cb(str, opts); else return str;
	}
	function dfl_exp(value) {
		if (cb) return cb(value, opts) else return value;
	}
	X.fileHandlers={};
