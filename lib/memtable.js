...
module.exports={install:function(X) {
	function mem(tbl, name, schema) {
		...
		for(let k in schema) {
			const v=schema[k];
			tbl.colmap.set(k,X.lookupdie(v,'memtable unknown column type')());
		}
		...
		let $mem = {
			...
			'get':function(rownum) {
				if(X.tarray(rownum)) return getMany(rownum);
			...
			'load':function(fn) { // TODO make this use a file handler for the whole table
				...
				const fns=X.each(tbl.colmap, function(col,k) { 
					return col.load(fn+"."+k);
				});
				return $mem.len() },
			'save':function(fn) {
				if(!fn) fn=X.homefile(ue(name));
				return X.each(tbl.colmap, function(col,k) { 
			...
			'upd':function(clauses,val) {
				if(!X.tdict(val)) return err('type','mem.upd',val);
				let ids=$mem.where(clauses);
				let tval=X.t(val);
				for(let idx in ids) {
					for(let k in val) {
						let col=tbl.colmap.get(k);
						if(!col) return err('key','mem.upd',k);
						col.set(ids[idx], val[k]);
					}
				}
				return ids;
			},
			'where':function(clauses) {
				...
				if(X.tarray(clauses)) return whereArray(clauses);
				const t=X.t(clauses);
