...
module.exports={install: function X(X){
	...
	*/
	...
	function deep0(x,f,opts,path) {
		...
		for(let i=0; i<xkl; i++) { 
			...
			if((opts.type!==X.U && tv===opts.type)
				  || (opts.key!==X.U && k==opts.key)) x[k]=f(v,join(path,k),opts,x);
			else if (tv==='array' || tv==='object') x[k]=deep0(v,f,opts,join(path,k));
			else if (opts.type===X.U) x[k]=f(v,join(path,k),opts,x);
		}
		return x;
	}
	...
	function each(x,f,opts) {
		const tx=t(x);
		if(tx==='array') return eachobj(x,f,opts);
		else if(tx==='object') {
	...
	function proj(f, x, y, z) { // curry function f with args x, y, or z.
		if (x!==U&&y!==U&&z!==U) return function(r) { return f(x, y, z, r); }.bind(f)
		else if (x===U&&y!==U) return function(r) { return f(r, y); }.bind(f)
		else if (x!==U&&y!==U) return function(r) { return f(x, y, r); }.bind(f)
		else if (x!==U) return function(r) { return f(x, r); }.bind(f)
		else return f;
	}
	...
	function key(x) { 
		...
		else if (tx==='object') return Object.keys(x);
	...
	impose('max member min range tail take');
